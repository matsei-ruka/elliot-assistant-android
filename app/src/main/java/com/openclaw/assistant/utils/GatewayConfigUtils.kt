    ...
    fun decodeGatewaySetupCode(rawInput: String): GatewaySetupCode? {
        ...
        return try {
            ...
            if (!NetworkUtils.isUrlSecure(url)) {
                Log.w("GatewayConfigUtils", "Setup code URL is not secure")
                return null
            }
            ...
            Log.d("GatewayConfigUtils", "Setup code decoded: hasToken=${token != null}, hasBootstrapToken=${bootstrapToken != null}, hasPassword=${password != null}")
            GatewaySetupCode(url = url, token = token, password = password, bootstrapToken = bootstrapToken)
        } catch (e: Throwable) {
            Log.w("GatewayConfigUtils", "Failed to decode setup code: ${e.javaClass.simpleName}")
            null
        }
    }
